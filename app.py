...
# Altera o título da aba se houver chamado aberto
...
# Atualiza a página automaticamente (30 seg)
...
def carregar_dados():
    ...
    try:
        df = pd.read_csv(DB_FILE)
        # Padroniza a data para string no formato BR
    ...
    if chamado_aberto.empty:
        ...
        if st.button("🔔 CHAMAR AGORA", type="primary"):
            if desc:
                ...
                novo = pd.DataFrame([{"ID": novo_id, "Célula": sel_ups, "Motivo": motivo, "Descrição": desc, "Início": get_brasil_time().strftime("%H:%M:%S"), "Fim": "-", "Status": "🔴 Aberto", "Data": hoje, "Ação": "-", "Minutos": 0.0}])
                pd.concat([dados, novo], ignore_index=True).to_csv(DB_FILE, index=False)
                st.rerun()
    else:
        st.markdown(f'<div class="piscante"><h1>⏳ AGUARDANDO ASSISTENTE...</h1><p style="font-size:20px;">Célula {sel_ups} | Motivo: {chamado_aberto.iloc[0]["Motivo"]}</p><p>Obs: {chamado_aberto.iloc[0]["Descrição"]}</p></div>', unsafe_allow_html=True)

# --- ABA 2: ASSISTENTE (Métricas Fixas) ---
with tabs[1]:
    if not st.session_state.logado:
        ...
        # MÉTRICAS COM PROTEÇÃO CONTRA VAZIO
        ...
        if not ativos.empty:
            ...
            st.success("✅ Nenhum chamado pendente.")

        with st.expander("🗑️ Opções de Limpeza"):
            if st.checkbox("Zerar contagem de hoje?"):
                if st.button("Confirmar Limpeza"):
                    df_res = dados[dados['Data'] != hoje]
                    df_res.to_csv(DB_FILE, index=False)
                    st.rerun()
...
with tabs[2]:
    if st.session_state.logado:
        if not dados.empty:
            ...
            sel_u = f_col2.multiselect("Células:", sorted(dados['Célula'].unique()))
            ...
            if not df_fig.empty:
                ...
                with g1:
                    ...
                    st.plotly_chart(px.bar(cont, x='Célula', y='Qtd', title="Paradas por UPS", color_discrete_sequence=['#ff4b4b']), use_container_width=True)
                with g2:
