...
st.markdown("""
    ...
    .alerta-piscante { animation: piscar 1s infinite; padding: 20px; border-radius: 10px; color: white !important; text-align: center; margin-bottom: 20px; font-weight: bold; }
...
if st.session_state.pagina_ativa == "📲 Terminal Operador":
    ...
    if aberto.empty:
        lista_p = ["Falta de Matéria-prima", "Qualidade", "Composer", "Abastecimento", "Processo", "Manutenção", "Outros"]
        motivo = c2.selectbox("Qual o problema?", lista_p)
        obs = st.text_input("Observação Adicional (Opcional):")
        if st.button("🔔 ENVIAR CHAMADO", type="primary"):
            final_d = f"{motivo} - {obs}" if obs else motivo
            nid = int(dados['ID'].max() + 1) if not dados.empty else 1
            novo = pd.DataFrame([{"ID": nid, "Célula": sel_ups, "Motivo": motivo, "Descrição": final_d, "Início": get_br_time().strftime("%H:%M:%S"), "Fim": "-", "Status": "🔴 Aberto", "Data": hoje, "Ação": "-", "Minutos": 0.0}])
    ...
    if not st.session_state.logado:
        ...
        if not ativos.empty:
            for i, r in ativos.iterrows():
                with st.expander(f"🔴 Célula: {r['Célula']} - Aberto às {r['Início']}", expanded=True):
